...
import { datingTipsCanonical } from "@/lib/about-section";
...
export const metadata: Metadata = {
  ...
  alternates: { canonical: datingTipsCanonical() },
  openGraph: { title: TITLE, description: DESCRIPTION, url: datingTipsCanonical(), images: [HERO_IMAGE] },
};

export default async function DatingTipsPage() {
  const expert = await getAuthorProfile("christian-m-haas");
  const faqGraph = buildMagazineFaqGraph({ items: faqItems, pageUrl: datingTipsCanonical(), pageName: "Häufige Fragen zu Dating-Tipps" });
          ...
          {TIPS.map((tip, index) => (
            <li key={tip.title} className="tips-item">
              ...
              <div className="tips-copy">
                ...
                <p>{tip.text}</p>
                {tip.premium ? (
                  <a className="button button-secondary" href={publicUrl("de", "/premium-mitgliedschaft.html")}>
