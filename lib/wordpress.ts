...
const AUDIO_BOILERPLATE =
  /^\s*Artikel kurz anhören\s*Die wichtigsten Punkte kurz und verständlich zusammengefasst\.\s*(?:Dein Browser unterstützt das Audio-Element nicht\.)?\s*/i;
...
function cleanExcerpt(html = "") {
  ...
  return text
    .replace(AUDIO_BOILERPLATE, "")
    .replace(/^Kurzantwort\s+/i, "")
    .replace(/\s*(?:\[…\]|\[\.\.\.\]|…|\.\.\.)\s*$/, "")
    .trim();
}
...
async function fetchWp<T>(path: string, params: Record<string, string | number | boolean> = {}) {
  ...
  const response = await fetch(`${MAGAZINE_API_BASE}${path}?${search.toString()}`, {
    headers: {
      "User-Agent": "Amigo fitness-liebe magazine migration",
    },
    next: { revalidate: 300 },
