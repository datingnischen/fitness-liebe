...
<style>
:root{color-scheme:light;--red:#e0b200;--dark:#1f2328;--green:#fbca08;--line:#ece6cf;--muted:#5b6068}*{box-sizing:border-box}body{margin:0;background:transparent;color:#1f2328;font-family:"Open Sans",Arial,sans-serif}.state{display:grid;min-height:285px;place-items:center;padding:20px;border:1px solid var(--line);border-radius:20px;background:#fffdf5;color:var(--dark);font-weight:700;text-align:center}.grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:12px}.tile{display:grid;min-width:0;gap:8px;padding:10px;border:1px solid var(--line);border-radius:18px;background:#fff;text-decoration:none;color:inherit;box-shadow:0 10px 24px rgba(31,35,40,.08);transition:.18s}.tile:hover,.tile:focus-visible{transform:translateY(-2px);border-color:var(--red);outline:none}.image{overflow:hidden;aspect-ratio:1;border-radius:14px;background:#f3f1ea}.image img{display:block;width:100%;height:100%;object-fit:cover}.tile strong,.tile span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.tile strong{font-size:.94rem}.tile span{color:var(--muted);font-size:.8rem}@media(max-width:700px){.grid{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:430px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
</style></head><body>
...
export function IconySinglesWidget(props: Props) {
  ...
  const controlName = `fitness-singles-${props.platformId}-${props.zip}`;

  return (
    <section className={styles.widget} aria-labelledby={`singles-${props.zip}`}>
      <div className={styles.copy}>
        <p className={styles.eyebrow}>Singles entdecken</p>
        <h2 id={`singles-${props.zip}`}>Neue sportliche Singles in {props.city}</h2>
      ...
      <div className={styles.actions}>
        ...
        <span>Kostenlos starten · Umkreis erweitern · aktive Menschen treffen</span>
      </div>
    </section>
  );
}
